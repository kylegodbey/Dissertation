...
%\documentclass[aps,prc,twocolumn,showpacs,superscriptaddress,longbibliography,floatfix,10pt]{revtex4-2}
%\documentclass[reprint,aps,prc,twocolumn,superscriptaddress,longbibliography,floatfix,10pt]{revtex4-2}

\chapter{Absence of hindrance in microscopic \textsuperscript{12}C+\textsuperscript{12}C fusion study}\label{chapters:chapter_2}
\vspace{-7mm}
...
%\thanks{}
%\altaffiliation{}
\affil[1]{Department of Physics and Astronomy, Vanderbilt University, Nashville, Tennessee 37235, USA}
...
\edef\oldrightskip{\the\rightskip}
	...
	% this could be ok to add? -> despite the methods not involving a fit to reaction data
...
% References should be done using the \protect\cite, \ref, and \label commands
...
%and \textsuperscript{12}C+\textsuperscript{30}Si~\protect\cite{montagnoli2018} 
...
%This possible resonance would be at an energy below current experimental capabilities, thus theoretical studies are important in examining this phenomenon~\protect\cite{umar2012a}. 


%References~\protect\cite{spillane2007,zickefoose2011} also represent two different techniques in obtaining fusion cross sections, namely via $\gamma$ ray and charged particle detection of the evaporation residues, respectively.

%Indeed, recent experimental results of \textsuperscript{13}C+\textsuperscript{12}C show an increasing S-factor at very low energies where the hindrance model predicts a sharp downturn~\protect\cite{zhang2018}.
...
%The potential is then used in calculating fusion cross sections as before, though excitation channels are considered in addition to the bare nuclear potential to provide a more robust picture of fusion~\protect\cite{esbensen2011,jiang2013}.
...
%early adiabatic time-dependent DHF (ATDHF) calculations~\protect\cite{heenen1981}.
%Even more, recent work uses the time-dependent wave-packet method in an attempt to reproduce the resonances that show in fusion of \textsuperscript{12}C+\textsuperscript{12}C~\protect\cite{diaz-torres2018}.
...
%After some large propagation time observables are calculated using the time-propagated wave functions~\protect\cite{diaz-torres2018}.
...
%This energy dependence has been investigated in carbon fusion, as discussed above, however TDHF naturally includes the shape effects arising from near-barrier collisions~\protect\cite{umar2014a,jiang2014} as opposed to a phenomenological approach.
...
\subsection{Density-constrained frozen Hartree-Fock}
\label{sec:dcfhf}
...
The DC-TDHF approach~\protect\cite{umar2006b} is then employed to calculate the ion-ion potentials $V_{\mathrm{DC{\hbox{-}}TDHF}}(R)$ directly from TDHF dynamics and has been used to calculate fusion cross sections for a wide range of
...
However, standard implementations of barrier penetration models, i.e., with incoming wave boundary conditions (IWBC) of absorbing imaginary potentials at short distance, cannot account for the sub-barrier resonances observed experimentally~\protect\cite{jiang2013}. 
...
\label{eq:se}
\end{equation}
The IWBC method is used to calculate transmission probabilities which assumes that fusion occurs once the minimum of V(R) is reached~\protect\cite{rawitscher1964}.
...
%In the current work, a transfer matrix approach has been used to incorporate the coordinate-dependent mass in the calculation of transmission probabilities via a discretization of M(R) and V(R)~\protect\cite{fedirko2010}.
